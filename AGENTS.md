...
- **`src/terms/base.py`** — `StepType` enum (`AUTOMATIC` / `CHOICE`), `StepStatus` enum (the machine-readable outcome tag on `StepOutcome.status`; a `str, Enum` so it serializes to its string value unchanged while an unknown status fails loudly at construction), `StepPrompt` (self-describing step metadata for the frontend), `SubmitResult` (uniform API response). Abstract `Step` (`step_id`, `step_type`, `prompt()`, `resolve(player_input)`, `apply()`) and `Term` (sequences steps, owns the `resolve → apply → advance` lifecycle via `submit()`).
