...
- **`src/terms/careers/`** — The career flow, split into focused sub-modules under a package whose `__init__.py` re-exports the full public surface (so `from src.terms.careers import <name>` still works):
  ...
  - **`aging.py`** — the official MgT 2022 `AGING_TABLE` (graded result rows) and `AgingStep`: a single 2D roll with a DM of −(total terms served), reductions applied to the physical characteristics (and Intelligence on the worst row).
